...
void static inline rtl_li(rtlreg_t* dest, uint32_t imm) {
  *dest = imm;
}
...
static inline void rtl_update_ZF(const rtlreg_t* result, int width) {
  ...
  //TODO();
  uint32_t temp_rtlh = 32 - width*8;

  if ((*result << temp_rtlh) >> temp_rtlh == 0)
	  cpu.flags.ZF = 1;
  else
	  cpu.flags.ZF = 0;
}
...
static inline void rtl_update_SF(const rtlreg_t* result, int width) {
  ...
  //TODO();
  /*if (*result >> (width * 8 - 1) == 0x1)//顺序错啦！rlgl
	  cpu.flags.SF = 1;
  else
	  cpu.flags.SF = 0;*/
  cpu.flags.SF = *result >> (width * 8 - 1);
}
